...
export async function fetchProfile(userId) {
  const { data, error } = await supabase
    .from("profiles")
    .select("id, username, display_name, bio, avatar_url, tip_url, hire_url, tool_links, social_links, pinned_creation_ids, created_at, auto_approve, founding_creator, credits, credits")
    .eq("id", userId)
    .maybeSingle();
...
export async function fetchProfileByUsername(username) {
  const { data, error } = await supabase
    .from("profiles")
    .select("id, username, display_name, bio, avatar_url, tip_url, hire_url, tool_links, social_links, pinned_creation_ids, created_at, founding_creator")
