...
function GeneratePage({ user, profile, notify, setPage, setGenSubmission, setProfile, genPrefill, setGenPrefill, setApPrefill, activeProject }) {
  const GEN_MODELS = [
    ...
    { key: "seedance-2.0-480", label: "Seedance 2.0 — Draft (480p)", costPerSecond: 3, durations: [5, 10, 15], ratios: ["16:9", "9:16", "1:1", "21:9"] },
    { key: "seedance-2.0", label: "Seedance 2.0 — Flagship (720p)", costPerSecond: 6, durations: [5, 10, 15], ratios: ["16:9", "9:16", "1:1", "21:9"] },
    { key: "seedance-2.5-480", label: "Seedance 2.5 — Draft (480p, up to 30s)", costPerSecond: 6, durations: [5, 10, 15, 30], ratios: ["16:9", "9:16", "1:1", "21:9"] },
    { key: "seedance-2.5", label: "Seedance 2.5 — Flagship (720p, up to 30s)", costPerSecond: 12, durations: [5, 10, 15, 30], ratios: ["16:9", "9:16", "1:1", "21:9"] },
 ...
 const IMAGE_MODELS = [
  ...
  useEffect(() => {
    ...
    locRef.current = genPrefill.locationData ? { data: genPrefill.locationData, name: genPrefill.locationName ?? null } : null;
            ...
            <div className="gen-form-row">
              <div className="gen-cost">
                ...
                {(() => {
                  ...
                  const LABELS = { "16:9": "16:9 — Landscape", "9:16": "9:16 — Vertical", "1:1": "1:1 — Square", "21:9": "21:9 — Ultrawide" };
                  return (
                    <select
...
const purchaseLoading = creation.is_premium && !purchasesLoaded; const priceLabel = creation.price_cents ? "$" + (creation.price_cents / 100).toFixed(2) : "$4.99";
  ...
  async function handleToggleComments() {
    setCmBusy(true);
    const { error } = await updateCommentsEnabled(creation.id, !commentsOn);
    setCmBusy(false);
    if (error) { notify("Could not update comments: " + error.message); return; }
    setCreations((prev) => prev.map((c) => c.id === creation.id ? { ...c, comments_enabled: !commentsOn } : c));
    notify(!commentsOn ? "Comments turned on." : "Comments turned off.");
  }
          ...
          {creation.is_premium && hasVideo && (
            purchased
              ...
              : purchaseLoading
                ...
                : <div className="unlock-area"><span className="unlock-label">Own this film &mdash; full-quality download, and you support the creator directly.</span><button className="btn-unlock-restrained" onClick={handleBuy} disabled={checkingOut} style={{ opacity: checkingOut ? 0.6 : 1 }}>
  {checkingOut ? "Opening checkout..." : "Buy Film \u2014 " + priceLabel}
</button></div>
          )}</>}
        </div>
