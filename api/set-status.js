...
const forClient = (assets) => assets && ({
  splat500k: assets.splats?.spz_urls?.["500k"] ?? null,
  splat100k: assets.splats?.spz_urls?.["100k"] ?? null,
  collider: assets.mesh?.collider_mesh_url ?? null,
  pano: assets.imagery?.pano_url ?? null,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (row.status === "ready" && row.world_id) {
      ...
      return res.status(200).json({ status: "ready", name: row.name, assets: forClient(assets) });
    }
