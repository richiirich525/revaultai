import { createClient } from "@supabase/supabase-js";
import Mux from "@mux/mux-node";

export default async function handler(req, res) {
  if (req.method !== "POST") {
    return res.status(405).json({ error: "Method not allowed" });
  }

  // Auth check
  const authHeader = req.headers.authorization ?? "";
  const token = authHeader.startsWith("Bearer ") ? authHeader.slice(7) : null;
  if (!token) {
    return res.status(401).json({ error: "Unauthorized." });
  }
  try {
    const supabase = createClient(
      process.env.VITE_SUPABASE_URL,
      process.env.VITE_SUPABASE_ANON_KEY
    );
    const { data: { user }, error } = await supabase.auth.getUser(token);
    if (error || !user) {
      return res.status(401).json({ error: "Unauthorized: invalid or expired session." });
    }
  } catch {
    return res.status(401).json({ error: "Unauthorized: could not verify session." });
  }

  const { videoPublicUrl } = req.body;
  if (!videoPublicUrl) {
    return res.status(400).json({ error: "videoPublicUrl is required." });
  }

  const muxTokenId = process.env.MUX_TOKEN_ID;
  const muxTokenSecret = process.env.MUX_TOKEN_SECRET;

  if (!muxTokenId || !muxTokenSecret) {
    // Mux not configured — return fallback
    return res.status(200).json({
      video_url: videoPublicUrl,
      preview_video: videoPublicUrl,
      thumbnail_image: "https://images.unsplash.com/photo-1462331940025-496dfbfc7564?w=1200&q=90",
    });
  }

  try {
    const mux = new Mux({ tokenId: muxTokenId, tokenSecret: muxTokenSecret });
    const { video } = mux;

    const asset = await video.assets.create({
      input: [{ url: videoPublicUrl }],
      playback_policy: ["public"],
      mp4_support: "standard",
    });

    // Poll until ready (max 120s)
    const maxWait = 120000;
    const pollInterval = 4000;
    const start = Date.now();
    let readyAsset = null;

    while (Date.now() - start < maxWait) {
      const current = await video.assets.retrieve(asset.id);
      if (current.status === "ready") { readyAsset = current; break; }
      if (current.status === "errored") throw new Error("Mux asset processing failed.");
      await new Promise((r) => setTimeout(r, pollInterval));
    }

    if (!readyAsset) throw new Error("Mux processing timed out.");

    const playbackId = readyAsset.playback_ids?.[0]?.id;
    if (!playbackId) throw new Error("No Mux playback ID returned.");

    return res.status(200).json({
      video_url:       videoPublicUrl,
      stream_url:      `https://stream.mux.com/${playbackId}.m3u8`,
      preview_video:   `https://image.mux.com/${playbackId}/animated.gif?start=0&end=4&width=640`,
      thumbnail_image: `https://image.mux.com/${playbackId}/thumbnail.jpg?time=0&width=1200`,
      mux_asset_id:    asset.id,
      mux_playback_id: playbackId,
    });
  } catch (err) {
    console.error("[process-video] Mux error:", err.message);
    // Fallback gracefully
    return res.status(200).json({
      video_url:       videoPublicUrl,
      preview_video:   videoPublicUrl,
      thumbnail_image: "https://images.unsplash.com/photo-1462331940025-496dfbfc7564?w=1200&q=90",
      mux_error:       err.message,
    });
  }
}